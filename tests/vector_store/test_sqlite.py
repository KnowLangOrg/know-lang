...
class TestSqliteVectorStore:
    ...
    def test_initialize_connection_error(self, mock_connect, sqlite_store):
        ...
        with pytest.raises(
            VectorStoreInitError,
            match="Failed to initialize SqliteVectorStore: Connection failed",
        ):
            sqlite_store.initialize()
    ...
    @patch("sqlite3.connect")
    def test_initialize_load_extension_error(
        self, mock_connect, sqlite_store, mock_sqlite3_conn
    ):
        """Test VectorStoreInitError if sqlite-vec extension fails to load."""
        mock_db_conn, _ = mock_sqlite3_conn
        mock_connect.return_value = mock_db_conn
        mock_db_conn.load_extension.side_effect = sqlite3.OperationalError(
            "Cannot load extension"
        )

        with pytest.raises(
            VectorStoreInitError, match="Failed to load sqlite-vec extension"
        ):
            sqlite_store.initialize()
        mock_db_conn.enable_load_extension.assert_called_once_with(True)
    ...
    async def test_add_documents_success(
        self, mock_connect_unused, sqlite_store, mock_sqlite3_conn, mock_app_config
    ):
        ...
        mock_cursor.execute.assert_any_call(
            expected_sql_main1,
            ("id1", metadatas[0][actual_content_field], packed_embedding1, meta_json1),
        )

        # Call 2: Insert into virtual table (doc1)
        expected_sql_virtual1 = (
            f"INSERT INTO {virtual_table_name} (rowid, embedding) VALUES (?, ?)"
        )
        mock_cursor.execute.assert_any_call(
            expected_sql_virtual1, (101, packed_embedding1)
        )
        ...
        mock_cursor.execute.assert_any_call(
            expected_sql_main2,
            ("id2", metadatas[1][actual_content_field], packed_embedding2, meta_json2),
        )

        expected_sql_virtual2 = (
            f"INSERT INTO {virtual_table_name} (rowid, embedding) VALUES (?, ?)"
        )
        mock_cursor.execute.assert_any_call(
            expected_sql_virtual2, (102, packed_embedding2)
        )  # Assuming lastrowid was updated to 102
    ...
    async def test_add_documents_mismatched_lengths(self, sqlite_store):
        ...
        with pytest.raises(
            ValueError,
            match="documents, embeddings, and metadatas lists must have the same length",
        ):
            await sqlite_store.add_documents(["doc1"], [[0.1]], [], None)

        with pytest.raises(
            ValueError,
            match="If provided, ids list must have the same length as documents",
        ):
            await sqlite_store.add_documents(
                ["doc1"], [[0.1]], [{"f": 1}], ["id1", "id2"]
            )

    @pytest.mark.asyncio
    async def test_query_success(
        self, sqlite_store, mock_sqlite3_conn, mock_app_config
    ):
        ...
        assert (
            results[0].document == "Document 1 content"
        )  # 'document' as per current accumulate_result
    ...
    def assert_sql_executed(self, mock_cursor, expected_sql):
        ...
        assert (
            normalized_expected_sql in executed_sqls
    ...
    def assert_sql_executed_multiple(self, mock_cursor, expected_sqls):
        ...
        for expected_sql in expected_sqls:
            ...
            assert (
                normalized_expected in executed_sqls_normalized
            ), f"Expected SQL '{normalized_expected}' not found in executed SQLs: {executed_sqls_normalized}"

    @patch("sqlite3.connect")
    def test_initialize_default_content_field(
        self, mock_connect, sqlite_store, mock_sqlite3_conn, mock_app_config
    ):
    ...
    @pytest.mark.asyncio
    @patch("sqlite3.connect")
    async def test_add_documents_no_content_field_in_metadata(
        self, mock_connect_unused, sqlite_store, mock_sqlite3_conn, mock_app_config
    ):
        ...
        actual_content_field_name_in_db = (
            mock_app_config.db.content_field
        ...
        mock_cursor.execute.assert_any_call(
            expected_sql_main1, (ids[0], docs_content[0], packed_embedding1, meta_json1)
        )
        ...
        mock_cursor.execute.assert_any_call(
            expected_sql_main2, (ids[1], docs_content[1], packed_embedding2, meta_json2)
        )
    ...
    @pytest.mark.asyncio
    @patch("sqlite3.connect")
    async def test_add_documents_auto_generated_ids(
        self, mock_connect_unused, sqlite_store, mock_sqlite3_conn, mock_app_config
    ):
        ...
        with patch.object(
            uuid, "uuid4", return_value=MagicMock(hex="testuuid123")
        ) as mock_uuid:
        ...
        mock_uuid.return_value = (
            mock_uuid_obj  # Make uuid.uuid4() return a specific UUID object
        )
        ...
        assert (
            found_call
        ), "Call to insert with auto-generated ID not found or params incorrect."

        # Check virtual table insert
        expected_sql_virtual = (
            f"INSERT INTO {virtual_table_name} (rowid, embedding) VALUES (?, ?)"
        )
        ...
        for call in mock_cursor.execute.call_args_list:
            sql, params = call[0]
            if (
                sql == expected_sql_virtual and params[0] == mock_cursor.lastrowid
            ):  # lastrowid would be 201
                assert params[1] == packed_embedding
                found_virtual_call = True
                break
        assert (
            found_virtual_call
        ), "Call to insert into virtual table with auto-generated ID not found."
    ...
    async def test_add_documents_sqlite_error(self, sqlite_store, mock_sqlite3_conn):
        ...
        with pytest.raises(
            VectorStoreError, match="Failed to add documents: Insert failed"
        ):
    ...
    async def test_get_document_success(
        ...
        assert (
            result.document == expected_content
        )  # 'document' based on accumulate_result
    ...
    async def test_get_document_sqlite_error(self, sqlite_store, mock_sqlite3_conn):
        ...
        with pytest.raises(
            VectorStoreError, match="Failed to get document: Get failed"
        ):
    ...
    async def test_update_document_success(
        ...
        await sqlite_store.update_document(
            doc_id, new_content, new_embedding, new_metadata
        )
        ...
        mock_cursor.execute.assert_any_call(
            f"SELECT rowid FROM {sqlite_store.table_name} WHERE id = ?", (doc_id,)
        )
        ...
        mock_cursor.execute.assert_any_call(
            expected_update_sql, (new_content, packed_new_embedding, meta_json, doc_id)
        )

        # 3. Check DELETE from virtual table
        mock_cursor.execute.assert_any_call(
            f"DELETE FROM {virtual_table_name} WHERE rowid = ?", (123,)
        )

        # 4. Check INSERT into virtual table
        mock_cursor.execute.assert_any_call(
            f"INSERT INTO {virtual_table_name} (rowid, embedding) VALUES (?, ?)",
            (123, packed_new_embedding),
        )
    ...
    async def test_update_document_not_found(self, sqlite_store, mock_sqlite3_conn):
        ...
        with pytest.raises(
            VectorStoreError, match="Document with id missing_doc not found for update."
        ):
            await sqlite_store.update_document("missing_doc", "content", [0.1], {})

    @pytest.mark.asyncio
    async def test_update_document_sqlite_error_on_update(
        self, sqlite_store, mock_sqlite3_conn
    ):
        ...
        with pytest.raises(
            VectorStoreError, match="Failed to update document: Update failed"
        ):
    ...
    async def test_get_all_sqlite_error(self, sqlite_store, mock_sqlite3_conn):
        ...
        with pytest.raises(
            VectorStoreError, match="Failed to get all documents: Get all failed"
        ):
    ...
    async def test_close_error(self, sqlite_store, mock_sqlite3_conn):
        ...
        with pytest.raises(
            VectorStoreError, match="Failed to close connection: Close failed"
        ):
            await sqlite_store.close()
    ...
    @pytest.mark.asyncio
    async def test_query_with_filter_success(
        self, sqlite_store, mock_sqlite3_conn, mock_app_config
    ):
        ...
        db_results = [
            (
                "id1",
                "content1",
                json.dumps({"meta_field": "valueA", "common": "yes"}),
                0.1,
            ),
            (
                "id2",
                "content2",
                json.dumps({"meta_field": "valueB", "common": "yes"}),
                0.2,
            ),
            (
                "id3",
                "content3",
                json.dumps({"meta_field": "valueA", "common": "no"}),
                0.3,
            ),
        ]
        mock_cursor.fetchall.return_value = db_results
        ...
        results = await sqlite_store.query(
            query_embedding, top_k=top_k, filter=query_filter
        )
        ...
        results_common = await sqlite_store.query(
            query_embedding, top_k=top_k, filter=query_filter_common
        )
    ...
    async def test_delete_success(
        ...
        mock_cursor.execute.assert_any_call(
            f"SELECT rowid FROM {sqlite_store.table_name} WHERE id = ?", ("id1",)
        )
        mock_cursor.execute.assert_any_call(
            f"SELECT rowid FROM {sqlite_store.table_name} WHERE id = ?", ("id2",)
        )

        # Check delete from main table (uses IN clause)
        expected_delete_main_sql = (
            f"DELETE FROM {sqlite_store.table_name} WHERE id IN (?,?)"
        )
        # Check delete from virtual table (uses IN clause)
        expected_delete_virtual_sql = (
            f"DELETE FROM {virtual_table_name} WHERE rowid IN (?,?)"
        )
        ...
        for call_args in mock_cursor.execute.call_args_list:
            ...
            if "DELETE FROM " + sqlite_store.table_name in sql and sql.endswith(
                "IN (?,?)"
            ):
                assert sorted(params) == sorted(
                    ids_to_delete
                )  # Order in IN clause might vary
        ...
        assert (
            virtual_delete_called_correctly
        ), "Virtual table delete not called correctly"
    ...
    async def test_delete_ids_not_found(self, sqlite_store, mock_sqlite3_conn):
        ...
        for call_args in mock_cursor.execute.call_args_list:
            ...
            if (
                "DELETE FROM " + sqlite_store.table_name + "_vec_idx" in sql
                and "IN (?)" in sql
            ):  # Only one rowid found
                assert params == (101,)
    ...
    async def test_delete_sqlite_error(self, sqlite_store, mock_sqlite3_conn):
        ...
        with pytest.raises(
            VectorStoreError, match="Failed to delete documents: Delete failed"
        ):
